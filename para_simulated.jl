...
function getSimulatedArray(n::Int64,p::Float64,d::Float64,epoch::Float64)
        TIME = zeros(n)
        for i=1:n
                TIME[i] += i
        end
        FLUX = randn(n)*0.1
        e = [epoch+p*i for i=0:ifloor((n-50)/p)]           # all the starting time of transits
...
(TIME,FLUX) = getSimulatedArray(1000,400.,25.,50.)
